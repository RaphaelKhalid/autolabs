...
export default function ExperimentsPage() {
  return <main className="archive-page">
    <nav className="archive-nav" aria-label="Primary"><Link href="/">A / AUTOLABS</Link><Link href="/events">Events</Link><Link href="/journal">Lab journal</Link><Link href="/research">Research</Link></nav>
    ...
    <section aria-label="Experiments">{experiments.map((experiment) => <article className="experiment-entry" key={experiment.slug}>
      <div className="archive-kicker">{experiment.label} / {experiment.status}{experiment.protocolState === 'locked' ? ' / protocol locked' : ''} / {experiment.scheduleLabel ?? `${experiment.rounds} rounds`}</div>
      ...
      <Link className="archive-button" href={`/experiments/${experiment.slug}`}>{experiment.status==='complete'?'Read the results':experiment.status==='active'?'Open live study':'View experiment'}</Link>
    </article>)}</section>
    <section className="archive-section"><h2>Run an event</h2><p>Preview one of six Afterlight protocols or make the single fixed test call before any sponsored study begins.</p><Link className="archive-button" href="/events">Open Events</Link></section>
