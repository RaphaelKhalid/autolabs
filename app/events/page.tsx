...
function TestCard({ onRun, running }: { onRun: (key: string) => void; running: boolean }) {
  ...
  return <div className="event-deck-card event-test-card">
    ...
    <div className="event-ledger"><div><span>MODEL</span><b>gpt-5.6-luna</b></div><div><span>SAMPLE</span><b>1 call</b></div><div><span>OUTPUT CAP</span><b>32 tokens</b></div><div><span>SPEND GUARD</span><b>One request</b></div></div>
...
export default function EventsPage() {
  ...
  async function runTest(ownerKey: string) {
    ...
    try {
      const response = await fetch('/api/test', { method: 'POST', headers: { 'x-autolabs-owner-key': ownerKey } });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'The test call failed.');
      setResult({ text: data.output ?? data.text ?? 'The provider returned an empty test result.', model: data.model ?? 'gpt-5.6-luna', requestId: data.requestId });
  ...
  return <main className="events-page">
    <style jsx global>{`
      ...
      .events-brand{display:flex;align-items:center;gap:8px}.events-nav{height:76px;border-bottom:1px solid rgba(23,27,25,.35);display:flex;align-items:center;justify-content:space-between;font:650 9px var(--font-mono),monospace;letter-spacing:.15em;text-transform:uppercase}.events-nav-links{display:flex;gap:22px;color:#5e6259}.events-nav a:hover{color:#9a4f36}
