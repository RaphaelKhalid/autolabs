...
export function ExperimentWorkbench({selfHosted=false,questionContext=null}:{selfHosted?:boolean;questionContext?:AfterlightQuestion|null}) {
  ...
  return <main className="workbench">
    ...
    {questionContext && <section className="wb-question-context" aria-label="Research question context"><div><span>RESEARCH HANDOFF</span><h2>{questionContext.title}</h2><a href={questionContext.sourceUrl} target="_blank" rel="noreferrer">Source record ↗</a>{questionContext.id === 'q-unsupervised-persona' && <a href="/experiments/persona-discovery">Open the SAE study ↗</a>}</div><p><strong>Evaluator pending.</strong> This source and question are design context. They do not create a runnable evaluator or start a paid run.</p></section>}
