...
const money=(value:number|null|undefined)=>value==null?'Awaiting prior accounting':`$${value.toFixed(4)}`;
...
export function Compatibility22Lab(){
  ...
  return <main className="reward-page compatibility21-page categories22-page">
    ...
    <section className="reward-overview" aria-label="Live category study"><div className={`reward-researcher ${running?'is-running':''}`}>
      ...
      <p className="compatibility21-eta">{error?'ETA unavailable while reconnecting':status?.status==='waiting'?'Starts after 002.1 accounting completes':compatibility21Eta(status?{status:status.status,etaSeconds:status.etaSeconds}:null)}</p><p className="reward-caption">Elapsed-pace estimate; updated every 15 seconds while visible.</p>
    ...
    <section className="reward-section"><p className="reward-label">SHARED $40 CEILING</p><h2>{status?.budget.totalCommittedUsd==null?'Prior commitments are accounted before launch':`${money(status.budget.totalCommittedUsd)} committed`}</h2><div className="compatibility21-budget"><div><span>002 + 002.1 conservative commitment</span><strong>{money(status?.budget.priorCommittedUsd)}</strong></div><div><span>002.2 conservative accounted spend</span><strong>{money(status?.budget.spentUsd)}</strong></div><div><span>Pending reservations</span><strong>{money(status?.budget.reservedUsd)}</strong></div></div>{(status?.budget.knownSpendUsd!==undefined||status?.budget.uncertainChargeUpperBoundUsd!==undefined)&&<p className="reward-caption">Within 002.2 accounted spend: {money(status?.budget.knownSpendUsd)} from reported usage; {money(status?.budget.uncertainChargeUpperBoundUsd)} maximum allowance for requests with unknown charges. This allowance is not a confirmed provider charge.</p>}<p className="reward-caption">Prior commitments may include allowances for unknown charges. No new $40 allocation. The entire fixed plan must fit the remaining shared budget before launch. The page is read-only and does not run or bill model calls.</p></section>
    ...
    <section className="reward-section"><p className="reward-label">CATEGORY RESULTS</p><h2>{sealed?'Held-out analysis is sealed':'Reference-relative category estimates'}</h2>
      <p className="reward-warning">Design limit: with 64 pairs, the frozen distribution-free bounds cannot establish population outcome equivalence within ±5 percentage points, regardless of the observed results. Checked witnesses and observed equivalence remain reportable; they do not establish population orthogonality.</p>
